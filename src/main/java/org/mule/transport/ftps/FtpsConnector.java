...
public class FtpsConnector extends AbstractConnector
{
     ...
     */
    public static final String DEFAULT_FTP_CONNECTION_FACTORY_CLASS = "org.mule.transport.ftps.FtpsConnectionFactory";
    ...
    public FTPSClient getFtp(EndpointURI uri) throws Exception
    {
        if (logger.isDebugEnabled())
        {
            logger.debug(">>> retrieving client for " + uri);
        }
        return (FTPSClient) getFtpPool(uri).borrowObject();
    }

    public void releaseFtp(EndpointURI uri, FTPSClient client) throws Exception
    {
        if (logger.isDebugEnabled())
        {
    ...
    public void destroyFtp(EndpointURI uri, FTPSClient client) throws Exception
    {
        if (logger.isDebugEnabled())
        {
        ...
        try
        {
            ...
            if (!FtpsConnectionFactory.class.isAssignableFrom(objectFactoryClass))
            {
                throw new InitialisationException(MessageFactory.createStaticMessage(
                        "FTP connectionFactoryClass is not an instance of org.mule.transport.ftp.FtpsConnectionFactory"),
                        this);
            }
        }
     ...
     */
    public void setupFileType(FTPSClient client, ImmutableEndpoint endpoint) throws Exception
    {
        int type;

        // well, no endpoint URI here, as we have to use the most common denominator
        // in API :(
        final String binaryTransferString = (String)endpoint.getProperty(FtpsConnector.PROPERTY_BINARY_TRANSFER);
